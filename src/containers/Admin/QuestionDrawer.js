...
function QuestionDrawer({isOpen, isClosed, questionObject}) {
    ...
    return ( 
        ...
        <DrawerContent>
          {(onClose) => (
            <>
              <DrawerHeader className="flex flex-col gap-1">{questionObject.prompt}</DrawerHeader>

              <DrawerBody >
                <div>  
                    <span className="text-default-300 text-small">Prompt: {questionObject.prompt}</span>
                    &nbsp;
                </div>
                <Form className="questionDrawerForm"
                onSubmit={(e) => {
                    e.preventDefault();
                    ...
                    variant="bordered"
                    />
                    <Button fullWidth type="submit" variant="solid" color="secondary" onPress={onClose}>
                        Update Prompt
                    </Button>
                </Form>
                <div>  
                    <span className="text-default-300 text-small">Choices: {questionObject.choices}</span>
                    &nbsp;
                </div>
                <Form className="questionDrawerForm"
                onSubmit={(e) => {
                ...
                <div>  
                    <span className="text-default-300 text-small">Correct Choice: {questionObject.correctChoice}</span>
                    &nbsp;
                </div>
                <Form className="questionDrawerForm"
                onSubmit={(e) => {
              ...
              <DrawerFooter className="flex flex-col gap-1" >
                    <Button className="w-full" type="submit" color="danger" variant="flat" onPress={() => deleteQuestion(questionObject.id)}>
                        Delete Question
                    </Button>
              </DrawerFooter>
            </>
